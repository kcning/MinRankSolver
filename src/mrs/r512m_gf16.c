...
r512m_gf16_memsize(uint32_t rnum) {
    return sizeof(R512MGF16) + sizeof(Grp512GF16) * rnum;
}
...
r512m_gf16_create(uint32_t rnum) {
    static_assert(sizeof(Grp512GF16) == 256, "size of Grp512GF16 is not 256");
    // NOTE: field 'rows' needs to be aligned to a 64-byte boundary
    R512MGF16* m = aligned_alloc(64, r512m_gf16_memsize(rnum));
    if(!m)
        return NULL;
...
r512m_gf16_at(const R512MGF16* m, uint32_t ri, uint32_t ci) {
    return grp512_gf16_at(r512m_gf16_raddr((R512MGF16*) m, ri), ci);
}
...
r512m_gf16_set_at(R512MGF16* m, uint64_t ri, uint64_t ci, gf16_t v) {
    grp512_gf16_set_at(r512m_gf16_raddr(m, ri), ci, v);
}
...
r512m_gf16_zero(R512MGF16* const m) {
    memset(m->rows, 0x0, sizeof(Grp512GF16) * r512m_gf16_rnum(m));
}
...
r512m_gf16_rand(R512MGF16* m) {
    for(uint32_t i = 0; i < r512m_gf16_rnum(m); ++i)
        grp512_gf16_rand(r512m_gf16_raddr(m, i));
}
...
r512m_gf16_copy(R512MGF16* restrict dst, const R512MGF16* restrict src) {
    assert(r512m_gf16_rnum(dst) == r512m_gf16_rnum(src));
    memcpy(dst->rows, src->rows, sizeof(Grp512GF16) * r512m_gf16_rnum(dst));
}
...
r512m_gf16_gramian(const R512MGF16* restrict m, RC512MGF16* restrict p) {
    rc512m_gf16_zero(p);
    for(uint64_t ri = 0; ri < r512m_gf16_rnum(m); ++ri) {
        const Grp512GF16* m_row = r512m_gf16_raddr((R512MGF16*) m, ri);

        for(uint32_t i = 0; i < 512; ++i) {
            // TODO: extract multiple coeffs at once
            gf16_t c = grp512_gf16_at(m_row, i);
            if(c == 0)
                continue;

            Grp512GF16* dst = rc512m_gf16_raddr(p, i);
            grp512_gf16_fmsubi_scalar(dst, m_row, c);
        }
    }
}
...
r512m_gf16_zc_pos(const R512MGF16* restrict m, uint512_t* restrict out) {
    uint512_t_max(out);
    for(uint32_t i = 0; i < r512m_gf16_rnum(m); ++i) {
        const Grp512GF16* row = r512m_gf16_raddr((R512MGF16*)m, i);
        uint512_t tmp; grp512_gf16_zpos(&tmp, row);
        uint512_t_andi(out, &tmp);
        if(uint512_t_is_zero(out))
            break;
    }
}
...
r512m_gf16_subset_zc_pos(const R512MGF16* restrict m,
    ...
    for(uint32_t i = 0; i < sz; ++i) {
        uint32_t ri = ridxs[i];
        const Grp512GF16* row = r512m_gf16_raddr((R512MGF16*)m, ri);
        uint512_t tmp; grp512_gf16_zpos(&tmp, row);
        uint512_t_andi(out, &tmp);
        if(uint512_t_is_zero(out))
            break;
...
r512m_gf16_fma(R512MGF16* restrict a, const R512MGF16* restrict b,
    ...
    assert(r512m_gf16_rnum(a) == r512m_gf16_rnum(b));
    for(uint32_t i = 0; i < r512m_gf16_rnum(a); ++i) {
        const Grp512GF16* b_row = r512m_gf16_raddr((R512MGF16*) b, i);
        Grp512GF16* dst = r512m_gf16_raddr(a, i);
        for(uint32_t j = 0; j < 512; ++j) {
            gf16_t coeff = grp512_gf16_at(b_row, j);
            if(coeff == 0)
                continue;
            const Grp512GF16* src = rc512m_gf16_raddr((RC512MGF16*)c, j);
            grp512_gf16_fmaddi_scalar(dst, src, coeff);
        }
    }
}
    ...
    for(uint32_t i = 0; i < r512m_gf16_rnum(a); ++i) {
        ...
        for(uint32_t j = 0; j < 512; ++j) {
            ...
            if(coeff == 0)
                continue;
            const Grp512GF16* src = rc512m_gf16_raddr((RC512MGF16*)c, j);
            grp512_gf16_fmaddi_scalar_mask(dst, src, coeff, d);
        }
    }
}
...
r512m_gf16_mixi(R512MGF16* restrict a, const R512MGF16* restrict b,
    ...
    for(uint32_t ri = 0; ri < r512m_gf16_rnum(a); ++ri) {
        Grp512GF16* dst = r512m_gf16_raddr(a, ri);
        const Grp512GF16* src = r512m_gf16_raddr((R512MGF16*)b, ri);
...
r512m_gf16_addi(R512MGF16* restrict a, const R512MGF16* restrict b) {
    assert(r512m_gf16_rnum(a) == r512m_gf16_rnum(b));
    for(uint64_t ri = 0; ri < r512m_gf16_rnum(a); ++ri) {
        Grp512GF16* dst = r512m_gf16_raddr(a, ri);
        const Grp512GF16* src = r512m_gf16_raddr((R512MGF16*)b, ri);
        grp512_gf16_addi(dst, src);
    }
}
